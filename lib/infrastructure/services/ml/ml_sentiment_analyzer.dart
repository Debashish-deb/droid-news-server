...
class MLSentimentAnalyzer {
  ...
  static const _banglaPositiveWeights = {
    'বিজয়': 1.0, 'জয়': 0.9, 'সাফল্য': 1.0, 'অর্জন': 0.9, 'উন্নয়ন': 0.8,
    'অগ্রগতি': 0.8, 'সমৃদ্ধি': 0.9, 'শান্তি': 0.8, 'প্রবৃদ্ধি': 0.8,
    'জিতেছে': 1.0, 'চ্যাম্পিয়ন': 1.0, 'সেঞ্চুরি': 0.9, 'হ্যাটট্রিক': 1.0,
    'ঐতিহাসিক': 0.9, 'ভালো': 0.6, 'সুন্দর': 0.7, 'চমৎকার': 0.9,
    'অসাধারণ': 0.9, 'দারুণ': 0.8, 'খুশি': 0.8, 'আনন্দ': 0.8,
    'উৎসব': 0.7, 'নিরাপদ': 0.6, 'সুস্থ': 0.6, 'ইতিবাচক': 0.7,
    'প্রশংসা': 0.7, 'পুরস্কার': 0.7, 'আশা': 0.6,
  };

  static const _banglaNegativeWeights = {
    'মৃত্যু': -1.0, 'হত্যা': -1.0, 'সংকট': -0.9, 'দুর্যোগ': -1.0,
    'বন্যা': -0.8, 'ঘূর্ণিঝড়': -0.9, 'বিপদ': -0.8, 'ক্ষতি': -0.7,
    'হার': -0.8, 'পরাজয়': -0.9, 'ব্যর্থ': -0.8, 'দুর্নীতি': -0.9,
    'ধর্ষণ': -1.0, 'নির্যাতন': -1.0, 'সহিংসতা': -0.9, 'দাঙ্গা': -0.9,
    'ধর্মঘট': -0.6, 'অবরোধ': -0.7, 'বিক্ষোভ': -0.6, 'আতঙ্ক': -0.9,
    'ভয়': -0.7, 'উদ্বেগ': -0.6, 'মন্দা': -0.8, 'মূল্যবৃদ্ধি': -0.7,
    'বেকারত্ব': -0.7, 'দারিদ্র্য': -0.8, 'অসুস্থ': -0.6, 'রোগ': -0.6,
    'খারাপ': -0.6, 'ভুল': -0.5, 'নেতিবাচক': -0.6,
  };

  // ── English sentiment (news-domain calibrated) ─────────────────────────────
  static const _englishPositiveWeights = {
    'victory': 1.0, 'win': 0.9, 'success': 1.0, 'achieve': 0.8,
    'breakthrough': 0.9, 'record': 0.7, 'champion': 1.0, 'celebrate': 0.8,
    'progress': 0.7, 'growth': 0.7, 'recovery': 0.7, 'improve': 0.6,
    'peace': 0.8, 'hope': 0.6, 'positive': 0.6, 'praised': 0.7,
    'award': 0.7, 'historic': 0.8, 'landmark': 0.7, 'safe': 0.5,
  };

  static const _englishNegativeWeights = {
    'death': -1.0, 'killed': -1.0, 'crisis': -0.9, 'disaster': -1.0,
    'flood': -0.8, 'cyclone': -0.9, 'collapse': -0.9, 'attack': -0.8,
    'violence': -0.9, 'corruption': -0.9, 'protest': -0.5, 'strike': -0.6,
    'loss': -0.7, 'defeat': -0.8, 'fail': -0.7, 'decline': -0.6,
    'inflation': -0.6, 'unemployment': -0.7, 'poverty': -0.8, 'fear': -0.7,
    'threat': -0.7, 'concern': -0.5, 'warning': -0.6, 'danger': -0.8,
    'riot': -0.9, 'murder': -1.0, 'rape': -1.0, 'abuse': -0.9,
  };

  // ── Negation & intensifiers ────────────────────────────────────────────────
  static const _banglaIntensifiers = {
    'অত্যন্ত': 1.5, 'খুব': 1.3, 'অনেক': 1.2, 'প্রচণ্ড': 1.5,
    'ভীষণ': 1.4, 'মারাত্মক': 1.5, 'অসাধারণ': 1.4, 'বেশি': 1.2,
  };

  static const _englishIntensifiers = {
    'very': 1.3, 'extremely': 1.5, 'highly': 1.3, 'massively': 1.4,
    'severely': 1.4, 'critically': 1.5, 'absolutely': 1.3,
  };

  static const _banglaNegators = [
    'না', 'নয়', 'নেই', 'ছিল না', 'হয়নি', 'পারেনি', 'করেনি',
  ];

  static const _englishNegators = [
    'not', "don't", "doesn't", "didn't", "won't", "no", "never", "without",
  ];
  ...
  SentimentResult _analyze(String text, _Language language) {
    return _analyzeStatic(text, language);
  }

  static SentimentResult _analyzeStatic(String text, _Language language) {
    final tokens = _tokenizeStatic(text);
    var positiveSum = 0.0;
    var negativeSum = 0.0;
    var hits = 0;

    final posWeights = language == _Language.bangla
        ? _banglaPositiveWeights
        : _englishPositiveWeights;
    final negWeights = language == _Language.bangla
        ? _banglaNegativeWeights
        : _englishNegativeWeights;
    final intensifiers = language == _Language.bangla
        ? _banglaIntensifiers
        : _englishIntensifiers;
    final negators = language == _Language.bangla
        ? _banglaNegators
        : _englishNegators;
  ...
  static double _sigmoidNormaliseStatic(double x) => (math.tan(x * 1.5) + 1) / 2;

  _Language _detectLanguage(String text) {
    return _detectLanguageStatic(text);
  }

  static _Language _detectLanguageStatic(String text) {
    final banglaChars = RegExp(r'[\u0980-\u09FF]').allMatches(text).length;
    final ratio = banglaChars / math.max(1, text.length);
    if (ratio > 0.4) return _Language.bangla;
    if (ratio > 0.1) return _Language.mixed;
...
enum _Language { english, bangla, mixed }
...
class SentimentResult {
  ...
  bool get isBangla => language == _Language.bangla;
