...
class SyncService {
  ...
  SyncService.disabled(this._observability, this._logger, this._prefs)
    : _premiumRepository = null,
      _firestore = null,
      _auth = null,
      _syncEnabled = false {
    _initialized = true;
    _logger.log(
      '🔄 SyncOrchestrator disabled until Firebase bootstrap is ready',
      level: Level.debug,
    );
  }
  ...
  bool get _canSyncPremium =>
      _syncEnabled &&
      _initialized &&
      _auth?.currentUser != null &&
      (_premiumRepository?.isPremium ?? false);

  bool get _canSyncBasic =>
      _syncEnabled && _initialized && _auth?.currentUser != null;
  ...
  Stream<Set<String>> get pendingFavoritesStream => _pendingController.stream;
  ...
  void _init() {
    ...
    _initialized = true;
    _logger.log('SyncService initialized', level: Level.debug);
    _observability.measure(
      'sync_init',
      () => _ensureLocalReady().then((_) => flushPending()),
    );
  }
  ...
  Future<void> _ensureLocalReady() async {
    if (_deviceId != null || _prefs == null) return;
  ...
  void _notifyPendingChanged() {
    _pendingController.add(getPendingFavoritesUrls());
  }

  Set<String> getPendingFavoritesUrls() {
    final delta = _readPendingDelta(_kPendingFavoritesDeltaKey);
    if (delta == null) return {};

    final upserts =
        (delta['upserts']?['articles'] as Map?)?.keys.cast<String>().toSet() ??
        {};
    final deletes =
        (delta['deletes']?['articles'] as Map?)?.keys.cast<String>().toSet() ??
        {};
  ...
  Stream<Map<String, dynamic>?>? settingsStream() {
    if (!_canSyncBasic || _uid == null) return null;
    flushPending();
  ...
  Stream<Map<String, dynamic>?>? favoritesStream() {
    if (!_canSyncPremium || _uid == null) return null;
    flushPending();
  ...
  Future<void> flushPending() async {
    if (_flushing) return;
    if (!_canSyncBasic || _uid == null) return;
  ...
  Future<bool> _applyFavoritesDeltaTransaction({
    ...
    try {
      await _firestore!.runTransaction((tx) async {
        ...
        final deletes = (delta['deletes'] is Map)
            ? (delta['deletes'] as Map).cast<String, dynamic>()
            : <String, dynamic>{};
        final upserts = (delta['upserts'] is Map)
  ...
  void _applyDeletesToNormalized(
    ...
    void applyEntity(String entity) {
      final entityDeletes = (deletes[entity] is Map)
  ...
  void _applyUpsertsToNormalized(
    ...
    void applyEntity(String entity) {
      final entityUpserts = (upserts[entity] is Map)
  ...
  Map<String, dynamic> _ensureFavoritesNormalized(Map<String, dynamic>? raw) {
    ...
    if (r['articlesByKey'] is Map && r['tombstones'] is Map) {
      return <String, dynamic>{
        ...
        'magazinesByKey': ((r['magazinesByKey'] as Map?) ?? {})
            .cast<String, dynamic>(),
        'newspapersByKey': ((r['newspapersByKey'] as Map?) ?? {})
            .cast<String, dynamic>(),
  ...
  void _log(String msg) {
    if (kDebugMode && !msg.contains('success') && !msg.contains('applied')) {
      _logger.info(msg);
    } else {
      _logger.log(msg, level: Level.debug);
    }
  }
}
