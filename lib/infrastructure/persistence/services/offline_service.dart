...
class OfflineService {
  ...
  static Future<bool> downloadArticle(NewsArticle article) async {
    try {
      ...
      // Enforce cache limit (configurable, defaulting to 100 here)
  ...
  static Future<void> _enforceMaxCacheSize(int maxLimit) async {
    try {
      ...
      if (count > maxLimit) {
        // Find oldest articles beyond the limit
        final excess = count - maxLimit;
        final oldArticles = await _database!.query(
          _tableName,
          columns: ['url', 'localImagePath'],
          orderBy: 'downloadedAt ASC',
          limit: excess,
        );

        for (final row in oldArticles) {
          final url = row['url'] as String;
          final localImagePath = row['localImagePath'] as String?;
  ...
  static Future<bool> clearAll() async {
    try {
      ...
      await logEvent(name: 'offline_cache_cleared');
