...
class SyncCoordinator {
  ...
  Future<void> syncFavorites({required SyncDirection direction}) async {
  ...
  Future<void> syncSettings({required SyncDirection direction}) async {
    _updateStatus(SyncStatus.syncing);
    try {
      switch (direction) {
        case SyncDirection.pull:
  ...
  Future<void> _pushFavoritesToCloud() async {
    ...
    for (final b in bookmarks) {
      final article = await (_database.select(
        _database.articles,
      )..where((t) => t.id.equals(b.articleId))).getSingleOrNull();
      if (article != null) {
  ...
  Future<void> _pushSettingsToCloud() async {
    // Safety guard:
    // Never push hardcoded defaults, as that can overwrite user preferences.
    // This coordinator does not own authoritative settings state.
    _logger.info(
      'Skipped settings push in SyncCoordinator (source of truth is SyncOrchestrator)',
    );
  }
