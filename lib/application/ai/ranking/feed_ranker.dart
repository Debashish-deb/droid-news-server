...
class FeedRanker {
  ...
  double _calculateThreadScore(NewsThread thread) {
    final hoursOld = DateTime.now()
        .difference(thread.mainArticle.publishedAt)
        .inHours;
    double freshnessScore = 10.0 - (hoursOld * 0.1);
    if (freshnessScore < 0) freshnessScore = 0;

    // Personalization Weight using TF-IDF Engine
    final double interestScore = _interestService.getPersonalizationScore(
      thread.mainArticle,
    );

    // Category Boosts (making previously unused methods useful)
    double categoryBoost = 1.0;
    if (_isLatest(thread)) categoryBoost *= 1.35;
    if (_isTrending(thread)) categoryBoost *= 1.45;
  ...
  bool _isLatest(NewsThread thread) {
    final article = thread.mainArticle;
    final category = article.category.toLowerCase();
    final hoursOld = DateTime.now().difference(article.publishedAt).inHours;
    return category.contains('latest') || hoursOld <= 12;
  }

  bool _isTrending(NewsThread thread) {
    final article = thread.mainArticle;
    final category = article.category.toLowerCase();
    final tags = article.tags?.map((t) => t.toLowerCase()).toList() ?? const [];
