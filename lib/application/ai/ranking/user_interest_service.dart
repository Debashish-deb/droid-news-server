...
class UserInterestService {
  ...
  static const int _kStateVersion = 2; 
  ...
  void _loadState() {
    try {
      ...
      if (savedVersion < _kStateVersion) {
        AppLogger.info('Invalidating old interest state (v$savedVersion -> v$_kStateVersion)');
        _vocabulary = [];
      ...
      if (vectorBase64 != null) {
        ...
        if (bytes.length % 2 == 0) {
           _currentInterestVector = Uint16List.fromList(bytes.buffer.asUint16List());
        } else {
           AppLogger.error('Interest vector corrupted: odd byte length (${bytes.length})');
           _currentInterestVector = null; 
        }
      }
    } catch (e) {
