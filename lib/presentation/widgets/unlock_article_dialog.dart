...
class _UnlockArticleDialogState extends ConsumerState<UnlockArticleDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isUnlocked = ref.read(rewardedAdServiceProvider).isArticleUnlocked(
      widget.articleUrl,
    );
    final bool adReady = ref.read(rewardedAdServiceProvider).isAdReady;
  ...
  Future<void> _showRewardedAd() async {
    ...
    final bool success = await ref.read(rewardedAdServiceProvider).showAdToUnlockArticle(
      widget.articleUrl,
    );
    ...
    if (success) {
      ...
      await ref.read(rewardedAdServiceProvider).loadAdManually();

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(loc.adNotReady), backgroundColor: Colors.orange),
      );
    }
  }
...
Future<bool> showUnlockDialog(
  ...
  final result = await showDialog<bool>(
    ...
    builder: (context) =>
        UnlockArticleDialog(articleUrl: articleUrl, articleTitle: articleTitle),
  );
  return result ?? false;
}
