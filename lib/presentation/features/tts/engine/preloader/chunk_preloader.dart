...
class ChunkPreloader with WidgetsBindingObserver {
  ChunkPreloader({required this.synthesizeChunk, this.bufferSize = 2}) {
  ...
  void didHaveMemoryPressure() {
    if (_preloadedPaths.isNotEmpty) {
      debugPrint(
        '[Preloader] Memory pressure detected. Clearing ${_preloadedPaths.length} buffer(s).',
      );
      clear();
    }
  }
  ...
  Future<void> preloadAhead({
    ...
    try {
      ...
      // Serialize synth calls to avoid flutter_tts file-generation races.
      for (final chunk in chunksToPreload) {
        await _preloadChunk(chunk);
      }
