...
import '../../infrastructure/services/config/feature_flag_service.dart';
...
final synthesisCircuitBreakerProvider = Provider<SynthesisCircuitBreaker>((
  ref,
) {
...
final vaultDatabaseProvider = Provider<VaultDatabase>(
  (ref) => ref.watch(di.vaultDatabaseProvider),
);
...
final assetsLoaderProvider = Provider<AssetsDataLoader>(
  (ref) => AssetsDataLoader(),
);

final publisherAssetsDataProvider = FutureProvider<Map<String, dynamic>>((
  ref,
) async {
...
final newspaperDataProvider = FutureProvider<List<dynamic>>((ref) async {
  final data = await ref.watch(publisherAssetsDataProvider.future);
  return (data['newspapers'] as List?)?.cast<dynamic>() ?? const [];
});

final magazineDataProvider = FutureProvider<List<dynamic>>((ref) async {
  final data = await ref.watch(publisherAssetsDataProvider.future);
  return (data['magazines'] as List?)?.cast<dynamic>() ?? const [];
});
