...
class SavedArticlesNotifier extends StateNotifier<SavedArticlesState> {
  ...
  Future<void> _init() async {
    await _service.init();
    unawaited(_service.runMaintenance(force: true));
  ...
  Future<void> loadSavedArticles() async {
    ...
    try {
      ...
      unawaited(_service.runMaintenance());
    } catch (e) {
  ...
  /// Count of entries that are older than 3 months.
  int get staleArticlesCount => _service.staleArticlesCount;

  /// Saved-at timestamp for a URL.
  DateTime? savedAtFor(String url) => _service.getSavedAt(url);

  /// Whether source URL was reachable during the latest maintenance run.
  bool isSourceLinkReachable(String url) => _service.isSourceLinkReachable(url);
