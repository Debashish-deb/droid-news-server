...
class MLSimilarityEngine {
  ...
  Future<void> initialize() async {
    ...
    // TODO: Load text embeddings model when available
    _isInitialized = true;
    if (kDebugMode) {
  ...
  Future<void> indexArticle(
    ...
    // Extract keywords for simple similarity
  ...
  Future<List<Map<String, dynamic>>> findSimilar(
    ...
    // Calculate similarity for each indexed article
    for (final entry in _articleIndex.entries) {
      final url = entry.key;
      if (url == currentUrl) continue; // Skip current article
