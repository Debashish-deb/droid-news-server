...
void main() {
  ...
  group('Premium Payment Flow Tests', () {
    ...
    group('Feature Unlocking', () {
      ...
      test('TC-PAYMENT-009: Pro unlocks all paid-only features', () async {
        final prefs = await SharedPreferences.getInstance();
        ...
        final hasOfflineSaving = tier == 'pro';
        final hasPremiumThemes = tier == 'pro';
        final hasNoAds = tier == 'pro';

        expect(hasOfflineSaving, true);
        expect(hasPremiumThemes, true);
        expect(hasNoAds, true);
      });
    ...
    group('Subscription Expiry', () {
      test('TC-PAYMENT-011: Expiry date set correctly', () async {
        ...
        expect(
          prefs.getInt('subscription_expiry'),
          greaterThan(DateTime.now().millisecondsSinceEpoch),
        );
      });
      ...
      test('TC-PAYMENT-013: Expired subscription downgraded', () async {
        ...
        if (isExpired) {
          await prefs.setBool('is_premium', false);
          await prefs.setString('current_subscription_tier', 'free');
        }

        expect(prefs.getBool('is_premium'), false);
        expect(prefs.getString('current_subscription_tier'), 'free');
      });
    });
    ...
    group('Subscription Management', () {
      ...
      test(
        'TC-PAYMENT-016: Switching paid SKU keeps Pro entitlement',
        () async {
          final prefs = await SharedPreferences.getInstance();

          await prefs.setBool('is_premium', true);
          await prefs.setString('current_subscription_tier', 'pro');
          await prefs.setString('active_product_id', 'pro_yearly_199');

          await prefs.setString('active_product_id', 'pro_lifetime_699');
          await prefs.setInt(
            'upgraded_at',
            DateTime.now().millisecondsSinceEpoch,
          );
          ...
          expect(prefs.getString('current_subscription_tier'), 'pro');
          expect(prefs.getString('active_product_id'), 'pro_lifetime_699');
          expect(prefs.getInt('upgraded_at'), greaterThan(0));
        },
      );

      test('TC-PAYMENT-017: Subscription downgrade to free handled', () async {
        final prefs = await SharedPreferences.getInstance();
