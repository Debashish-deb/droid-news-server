...
import 'package:bdnewsreader/core/telemetry/log_sanitizer.dart';
  ...
  group('Security Logging Tests', () {
    ...
    setUp(() {
      ...
      debugPrint = (String? message, {int? wrapWidth}) {
        final sanitized = LogSanitizer.sanitizeNullable(message);
        if (sanitized != null) {
          capturedLogs.add(sanitized);
        }
      };
    });
    ...
    test('debugPrint calls are removed in release mode', () {
      ...
      debugPrint = (String? message, {int? wrapWidth}) {
        callCount++;
        final sanitized = LogSanitizer.sanitizeNullable(message);
        if (sanitized != null) capturedLogs.add(sanitized);
      };
