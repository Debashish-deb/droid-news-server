...
class NewsNotifierMock extends NewsNotifier {
  NewsNotifierMock({
    ...
    state = NewsState(
      loading: loading ?? {'national': false, 'international': false, 'sports': false, 'entertainment': false},
      articles: articles ?? {},
      errors: errors ?? {},
    );
  }
...
void main() {
  group('Home Screen State Transitions', () {
    testWidgets('should show skeleton when loading takes time', (WidgetTester tester) async {
    ...
    testWidgets('should handle error state with retry button', (WidgetTester tester) async {
      final notifier = NewsNotifierMock(
